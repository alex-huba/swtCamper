...
public class UserReportServiceIntegrationTest {
    ...
    private UserReport createUserReport() {
        User operator = userServiceUnderTest.create("Operator",
                "password",
        ...
        User reporter = userServiceUnderTest.create("Max",
                "password",
                ...
                UserRole.PROVIDER,
                true);
        User reportee = userServiceUnderTest.create("Anna",
                "password",
        ...
        return userReportServiceUnderTest.create(new UserReport(reporter, reportee, "Das ist ein Test"));
    }
    ...
    @Test
    public void getUserReportByIdShouldReturnOrThrow() throws GenericServiceException {
        UserReport userReport = createUserReport();

        assertNotNull(userReportServiceUnderTest.getUserReportById(userReport.getId()));
        assertThrows(GenericServiceException.class, () -> userReportServiceUnderTest.getUserReportById(99L));
    }
    ...
    public void getAllUserReportsShouldReturnCorrectList() {
        ...
        userReportServiceUnderTest.create(new UserReport(userReport.getReportee(), userReport.getReporter(), "Das ist noch ein Test"));

        assertEquals(2, userReportServiceUnderTest.getAllUserReports().size());
        assertEquals("Max", userReportServiceUnderTest.getAllUserReports().stream().filter(userReportI -> userReportI.getReportReason().equals("Das ist noch ein Test")).collect(Collectors.toList()).get(0).getReportee().getUsername());
    }

    @Test
    public void acceptUserReportShouldBlockUserAndReturnInactiveUserReport() throws GenericServiceException {
        UserReport userReport = createUserReport();
        // does not block reportee...
        UserReport updatedUserReport = userReportServiceUnderTest.acceptUserReport(userReport.getId());
    ...
    @Test
    public void rejectUserReportShouldReturnInactiveUserReport() throws GenericServiceException {
        UserReport userReport = createUserReport();
        UserReport updatedUserReport = userReportServiceUnderTest.rejectUserReport(userReport.getId());
